...
class Phone extends Controller
{
    ...
    public function get(Request $request, Response $response): Response
    {
        ...
        $captcha = \InYota\Model\CaptchaPhone::byPhone($phone)->first();
        ...
        if ($captcha) {
            ...
            if ($es <= $s) {
                $response = new \InYota\Common\Message($response, false, sprintf('%s秒内只能发送一次短信（剩余：%ss）', $s, ($s - $es)));

                return $response->withJson();
            }

            // 删除（软删除）其他验证码
            \InYota\Model\CaptchaPhone::byPhone($phone)->delete();
        }

        $captcha = new \InYota\Model\CaptchaPhone();
        $captcha->phone = $phone;
        $captcha->captcha_code = rand(1000, 9999);
        $captcha->expires = 3600;

        if ($captcha->save()) {
            $response = new \InYota\Common\Message($response, true, '获取验证码成功！', $test == 1 ? $captcha->captcha_code : null);

        // 如果保存失败！
        } else {
            $response = new \InYota\Common\Message($response, false, '验证码获取失败！');
        }
    ...
    {
        $response = new \InYota\Common\Message($response, true, '验证码正确');
