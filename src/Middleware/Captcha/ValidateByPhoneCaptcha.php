...
namespace InYota\Middleware\Captcha;
...
class ValidateByPhoneCaptcha
{
    ...
    public function __invoke(Request $request, Response $response, callable $next)
    {
        ...
        // 验证验证码是否为空
            ...
            $response = new \InYota\Common\Message($response, false, '验证码不能为空');
        ...
        //  如果没有查询到数据
        if (!$captcha) {
            $response = new \InYota\Common\Message($response, false, '请先获取验证码');

            return $response->withJson();

        // 判断验证码是否正确
        } elseif ($captcha->captcha_code != $phoneCaptcha) {
            $response = new \InYota\Common\Message($response, false, '验证码不正确');

            return $response->withJson();

        // 判断验证码是否过期
        } elseif ($captcha->created_at->diffInSeconds(Carbon::now()) >= $captcha->expires) {
            $response = new \InYota\Common\Message($response, false, '验证码已经失效，请重新获取！');
